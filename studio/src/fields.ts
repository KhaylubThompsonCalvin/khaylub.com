...
export const collections: Record<string, CollectionSpec> = {
  ...
  music: {
    ...
    fields: {
      ...
      files: { kind: 'array-text', required: false, pattern: MEDIA_FILE_PATTERN, description: 'File names under public/media/<slug>/ (mp3, ogg); add the files by pull request until the media phase.' },
      external_url: { kind: 'url', required: false, description: 'Where the piece is hosted when it is not in the repository (decision D-07).' },
      cover: { kind: 'image', required: false, description: 'Cover art, 1600 px wide; needs the alt text below and the provenance group.' },
  ...
  video: {
    ...
    fields: {
      ...
      poster: { kind: 'image', required: true, description: 'The poster frame, 16:9, 1600 px wide.' },
      files: { kind: 'array-text', required: false, pattern: MEDIA_FILE_PATTERN, description: 'File names under public/media/<slug>/ (mp4, webm); add the files by pull request until the media phase.' },
      external_url: { kind: 'url', required: false, description: 'Where the video is hosted when it is not in the repository (decision D-07).' },
      ...
      cover: { kind: 'image', required: false, description: 'The card image when it differs from the poster; needs the alt text below.' },
      cover_alt: { kind: 'text', required: false, description: 'The cover for those who cannot see it; at least 5 characters when a cover is set.' },
      provenance: { kind: 'object', required: true, description: 'Credits: who made the recording and what is in frame.', fields: PROVENANCE_FIELDS },
