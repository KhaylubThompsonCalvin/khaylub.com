...
| `OWNER GATE / ACCEPTANCE PENDING` | implementation complete (pull request merged or built and stamped, no plan task open) but the canonical Owner Gate note (`AIDOCS/Phase <N> - Owner Gate.md`, see owner-gate.md) has a required item not SATISFIED, or the owner has not recorded acceptance. `/phase resume` stops here and points to `/phase gate`; `/phase close` completes the mechanics and refuses PASS |
...
A phase can be `PR MERGED` and still not accepted, and an owner message saying a requirement is complete does not make it so when the evidence fields are placeholders (Phase 12 on 2026-09-12: a PASS was recorded over bracketed template text and retracted the same day; the gate returned to `OWNER GATE / ACCEPTANCE PENDING`). More generally: a merge is the owner accepting the code into `main`, PASS is the owner closing the gate. Phase 12 on 2026-09-12 is the example: PR #17 merged, corrections on a `fix/` branch, acceptance still pending a human Journey 3 reader. Report both facts; never infer PASS from a merge.
